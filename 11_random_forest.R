...
# Run Lasso, just for comparison
# -------------------------------

par(mfrow=c(1,2))
plot(capen <- cv.gamlr(x=XXca, y=logMedVal, lmr=1e-6, standardize=FALSE))
plot(capen$gamlr)
round(coef(capen),2) ## Lasso gives pretty complicated model
...
# add importance="impurity" so that we store the variable importance information
...
# Extra: plot the predictions by location
# -------------------------------
...
pdf("slides/cart/CAHresiduals.pdf", width=8, height=4)
